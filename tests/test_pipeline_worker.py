...
def test_save_history_to_json_writes_timeframe_aware_files(tmp_path, monkeypatch):
    ...
    assert not (tmp_path / "history_BTC_USDT.json").exists()
...
def test_get_last_timestamp_queries_only_timeframe_rows(
    monkeypatch,
):
    captured_queries = []
    ...
    assert 'not exists r["timeframe"]' not in captured_queries[0]


def test_get_first_timestamp_queries_only_timeframe_rows(
    monkeypatch,
):
    captured_queries = []
