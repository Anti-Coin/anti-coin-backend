...
def static_export_candidates(
    ...
    Why:
    - 정적 산출물 계약을 canonical 단일 경로로 고정하기 위함이다.
    """
    canonical, _legacy = ctx._static_export_paths(
...
def save_history_to_json(ctx, df, symbol, timeframe):
    """
    history DataFrame을 canonical 정적 JSON으로 저장한다.
    ...
    try:
        ...
        canonical_path, _legacy_path = ctx._static_export_paths(
