...
| D-017 | P2 | `pipeline_worker.py` `_ctx()` 래퍼 패턴 해소 | done (2026-03-05) | 테스트가 `workers.*` 직접 참조로 전환되고 `_ctx()` 기반 orchestrator 위임 래퍼를 단계적으로 제거했다(`TS-011`). 회귀: `tests/test_pipeline_worker.py`, `tests/test_manifest_view.py`, `tests/test_api_status.py`, `pytest -q` 통과 |
...
| D-045 | P2 | Orchestrator 모듈화 인터페이스 잠금 | in_progress (2026-03-05) | 1차로 `policy_eval` 경계를 `utils/serve_policy.py`에 분리하고 manifest writer가 공통 evaluator(`evaluate_serve_allowed`)를 사용하도록 전환했다(`workers/export.py`). 2차로 `ingest_watermarks` I/O를 `utils/pipeline_runtime_state.py::IngestWatermarkStore`로 분리해 orchestrator의 state file 처리 경계를 저장소 계층으로 정규화했다(`scripts/pipeline_worker.py`). 3차로 model 경로 해석 규칙을 `utils/model_store.py`로 추출해 train/predict 양쪽이 동일 `model_io` resolver를 사용하도록 잠갔다(`scripts/train_model.py`, `workers/predict.py`). 4차로 prediction health 파일 read/write를 `utils/prediction_health_store.py`로 공통화해 API/worker 중복 로직을 축소했다(`api/main.py`, `workers/predict.py`). 5차로 dead wrapper(`_load/_save_prediction_health`, `_static_export_candidates`, `_extract_updated_at_from_files`, `_query_first/_query_last_timestamp`, `_fetch_ohlcv_paginated`, `_detect_gaps_from_ms_timestamps`, `_refill_detected_gaps`, `save_history_to_json`, `fetch_and_save`)를 제거하고 manifest health reader를 공통 store 유틸 직접 호출로 전환해 `_ctx` 의존면을 축소했다(`scripts/pipeline_worker.py`, `workers/export.py`, `tests/test_pipeline_worker.py`). 6차로 `workers.ingest` 내부에서 `ctx.get_*` 콜백 의존 일부를 모듈 직접 호출(`get_first/last_timestamp`, `get_exchange_latest_closed_timestamp`)로 바꿔 ctx 인터페이스 결합을 축소했다(`workers/ingest.py`). 7차로 `pipeline_worker`의 activation/detection 위임 래퍼(`get_exchange_earliest_closed_timestamp`, `evaluate_detection_gate_decision`, `build_symbol_activation_entry`)를 제거하고 orchestrator가 `workers.ingest`를 직접 호출하도록 전환했다(`scripts/pipeline_worker.py`, `tests/test_pipeline_worker.py`). 8차로 `pipeline_worker`의 publish 위임 래퍼(`update_full_history_file`, `upsert_prediction_health`)를 제거하고 `workers.export`/`workers.predict` 직접 호출로 경계를 단순화했다(`scripts/pipeline_worker.py`, `tests/test_pipeline_worker.py`). 9차로 `pipeline_worker`의 `run_prediction_and_save` 래퍼를 제거하고 `workers.predict` 직접 호출(`run_prediction_and_save_outcome`) 및 관련 테스트 경로를 정리했다(`scripts/pipeline_worker.py`, `tests/test_pipeline_worker.py`, `tests/test_model_contract.py`). 10차로 `pipeline_worker`의 `run_ingest_step` 래퍼를 제거하고 ingest outcome 경로가 `workers.ingest.fetch_and_save`를 직접 사용하도록 전환했다(`scripts/pipeline_worker.py`, `tests/test_pipeline_worker.py`). 11차로 `pipeline_worker`의 `build_runtime_manifest` 래퍼를 제거하고 manifest 생성 테스트를 `workers.export.build_runtime_manifest` 직접 호출로 전환했다(`scripts/pipeline_worker.py`, `tests/test_pipeline_worker.py`). 12차로 underfill 기준 계산에서 `pipeline_worker._minimum_required_lookback_rows` 래퍼를 제거하고 `workers.ingest.minimum_required_lookback_rows` 직접 호출로 경계를 단순화했다(`scripts/pipeline_worker.py`, `tests/test_pipeline_worker.py`). 13차로 underfill row count 조회에서 `pipeline_worker.get_lookback_close_count` 래퍼를 제거하고 `workers.ingest.get_lookback_close_count` 직접 호출로 경계를 단순화했다(`scripts/pipeline_worker.py`). 14차로 backward coverage 검사에서 `pipeline_worker.get_first_timestamp` 래퍼를 제거하고 `workers.ingest.get_first_timestamp` 직접 호출로 경계를 단순화했다(`scripts/pipeline_worker.py`, `tests/test_pipeline_worker.py`). 15차로 ingest precheck에서 `pipeline_worker.get_last_timestamp` 래퍼를 제거하고 `workers.ingest.get_last_timestamp` 직접 호출로 경계를 단순화했다(`scripts/pipeline_worker.py`, `tests/test_pipeline_worker.py`). 16차로 ingest since 판정에서 `pipeline_worker.resolve_ingest_since` 래퍼를 제거하고 `workers.ingest.resolve_ingest_since` 직접 호출로 경계를 단순화했다(`scripts/pipeline_worker.py`, `tests/test_pipeline_worker.py`). 17차로 manifest write 경로에서 `pipeline_worker.write_runtime_manifest` 래퍼를 제거하고 `workers.export.write_runtime_manifest` 직접 호출로 경계를 단순화했다(`scripts/pipeline_worker.py`, `workers/export.py`, `tests/test_pipeline_worker.py`). 선행 게이트 문서(`docs/PIPELINE_REFACTOR_TARGET_SELECTION.md`, `docs/PIPELINE_REFACTOR_DESIGN_BLUEPRINT.md`)를 잠갔고 Wave A(`TS-001/002/015`) + Wave B(`TS-003/009`) + Wave C(`TS-004/005`) + Wave D 1차(`TS-010`)를 완료했다. Wave D 2차(`TS-011`)를 완료했다. 회귀: `PYENV_VERSION=coin pytest -q tests/test_api_status.py tests/test_pipeline_worker.py tests/test_manifest_view.py`(90 passed), `PYENV_VERSION=coin pytest -q`(170 passed). 후속은 D-016/D-021 경로에서 orchestrator 조합 경계 고도화다. |
