...
| D-044 | P1 | 상태 스키마 정규화(`symbol_activation`/`prediction_health`) | in_progress (2026-03-05) | 1차로 `prediction_health`의 redundant identity(`symbol`, `timeframe`)를 제거해 key 기반 identity로 고정했다(`workers/predict.py`). 2차로 `symbol_activation`에서 `state`를 단일 SoT로 두고 `visibility/is_full_backfilled`는 state 기반 파생으로 정규화했다(`utils/pipeline_contracts.py`). 회귀: `tests/test_pipeline_worker.py::test_upsert_prediction_health_tracks_failure_and_recovery`, `tests/test_pipeline_worker.py::test_symbol_activation_snapshot_derives_flags_from_state_when_conflicted`, 전체 `pytest -q` 통과. 남은 게이트는 local compose 스모크다. |
| D-045 | P2 | Orchestrator 모듈화 인터페이스 잠금 | in_progress (2026-03-05) | 1차로 `policy_eval` 경계를 `utils/serve_policy.py`에 분리하고 manifest writer가 공통 evaluator(`evaluate_serve_allowed`)를 사용하도록 전환했다(`workers/export.py`). 2차로 `ingest_watermarks` I/O를 `utils/pipeline_runtime_state.py::IngestWatermarkStore`로 분리해 orchestrator의 state file 처리 경계를 저장소 계층으로 정규화했다(`scripts/pipeline_worker.py`). 3차로 model 경로 해석 규칙을 `utils/model_store.py`로 추출해 train/predict 양쪽이 동일 `model_io` resolver를 사용하도록 잠갔다(`scripts/train_model.py`, `workers/predict.py`). 대규모 리팩토링 선행 게이트로 `docs/PIPELINE_REFACTOR_TARGET_SELECTION.md`(대상 선별)과 `docs/PIPELINE_REFACTOR_DESIGN_BLUEPRINT.md`(공통모듈/인터페이스/추가 설계축)를 잠갔고, 다음 변경은 해당 matrix(`TS-*`, `CM-*`, `IF-*`) 승인 항목만 순차 실행한다. 회귀: `PYENV_VERSION=coin pytest -q tests/test_train_model.py tests/test_model_contract.py tests/test_pipeline_worker.py`(86 passed), `PYENV_VERSION=coin pytest -q`(167 passed). 후속은 orchestrator 호출면 슬림화(`pipeline_worker` 조합 경계 강화)다. |
...
| D-047 | P1 | Scheduler mode boundary 단일화(`poll_loop` 제거) | in_progress (2026-03-05) | `WORKER_SCHEDULER_MODE`를 boundary 단일값으로 잠그고(`scripts/worker_config.py`), `run_worker`가 `poll_loop`/invalid 설정에서 fallback 없이 즉시 `ValueError`로 실패하도록 고정했다(`scripts/pipeline_worker.py`). 회귀: `PYENV_VERSION=coin pytest -q tests/test_pipeline_worker.py`(63 passed, fail-fast 케이스 포함), `PYENV_VERSION=coin pytest -q`(163 passed). 남은 게이트는 local compose 스모크다. |
