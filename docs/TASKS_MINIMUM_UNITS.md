...
| D-044 | P1 | 상태 스키마 정규화(`symbol_activation`/`prediction_health`) | in_progress (2026-03-05) | 1차로 `prediction_health`의 redundant identity(`symbol`, `timeframe`) 필드를 제거해 key 기반 identity로 고정했다(`workers/predict.py`). 회귀: `tests/test_pipeline_worker.py::test_upsert_prediction_health_tracks_failure_and_recovery`에서 persisted payload에 identity 필드 미포함을 검증한다. 2차는 `symbol_activation`의 파생 필드 정규화(`state` 단일 SoT)다. |
