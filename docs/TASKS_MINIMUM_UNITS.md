...
| D-043 | P1 | Manifest 계약 분리(`manifest.v2` 단일 파일 내 `public`/`ops`) | in_progress (2026-03-05) | writer를 `manifest.v2`(`public`/`ops`)로 전환하고, admin consumer는 v2에서 `ops.entries`를 우선 읽도록 반영했다(`workers/export.py`, `admin/manifest_view.py`). 회귀: `PYENV_VERSION=coin pytest -q tests/test_pipeline_worker.py tests/test_manifest_view.py` + 전체 `pytest -q` 통과. 남은 게이트는 local compose 스모크와 FE consumer를 `public` 전용으로 잠그는 확인이다. |
