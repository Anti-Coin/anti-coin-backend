...
| C-006 | P1 | timeframe 경계 기반 trigger 전환 | open (gated) | `B-003` 검증 증거 확보 후 착수, 완료 시 불필요 cycle 감소 + 정합성 개선 |
...
| C-002 | C-005/C-006 의사결정을 위한 관측 근거가 현재 부족함 | 지표 부정확/과다로 잘못된 최적화 결정을 유도 | 실행시간/실패율/오버런 샘플 수집 + 오버헤드 측정 | 추가 메트릭 수집 비활성화 |
...
| C-006 | 고정 poll 루프는 경계 미스/불필요 cycle로 비용과 오탐을 증가시킴 | 경계 누락 또는 중복 트리거로 stale/중복 실행 발생 | 경계 시뮬레이션 테스트 + 1캔들당 1트리거 로그 검증 | 기존 고정 poll 루프로 복귀 |
