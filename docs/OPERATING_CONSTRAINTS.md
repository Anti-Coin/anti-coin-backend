...
## 2. 현재 배포 현실
1. 현재 배포 트리거는 `main push`(또는 수동 dispatch)로 제한한다.
2. `dev`는 배포 대상이 아니며, CI(테스트) 통합 브랜치로 사용한다.
3. 서비스 초기 단계라 운영자 1인 사용 시나리오가 중심이다.
4. 배포 전 로컬 스모크 게이트를 유지한다(`docker-compose.local.yml` 기반).
