...
|---|---|---|---|---|---|---|
| TS-001 | Model IO | `scripts/train_model.py` primary legacy model write(`model_{symbol}.json`) | REMOVE_NEXT | UC-PRED-01은 canonical 누락 fail-closed를 요구한다. runtime read가 canonical-only면 sidecar legacy write는 계약 노이즈다. | `tests/test_train_model.py`, `tests/test_model_contract.py`, local train smoke | 커밋 단위 revert |
| TS-002 | Model IO | `scripts/train_model.py` primary legacy metadata write(`model_{symbol}.meta.json`) | REMOVE_NEXT | TS-001과 동일. metadata도 canonical 단일 계약으로 맞춰 drift를 줄인다. | `tests/test_train_model.py` + metadata 파일 계약 검증 | 커밋 단위 revert |
...
| TS-015 | Docs Contract | `CONTEXT_MIN/PLAN/SESSION_HANDOFF`의 legacy fallback 서술 | REMOVE_NEXT | 코드 계약과 문서 계약이 어긋나면 잘못된 의사결정을 유도한다. | 문서 3종 동기화 diff review | 커밋 단위 revert |
...
1. Wave A (Low blast radius): TS-001, TS-002, TS-015
