...
| TS-011 | Orchestrator | `_ctx()` 기반 monkeypatch 호환 래퍼 군 | DONE (2026-03-05) | `D-017` 목표. 조합 책임만 남기기 위해 wrapper 축소가 필요했고, 단계별 테스트 계약 이관과 함께 orchestrator 위임 래퍼를 제거했다(`resolve_ingest_since/get_first/get_last/underfill helpers/run_ingest_step/run_prediction_and_save/build/write manifest` 포함). `workers.ingest` 내부에서도 `ctx.get_*` 콜백 의존 일부를 모듈 직접 호출(`get_first/last_timestamp`, `get_exchange_latest_closed_timestamp`)로 축소해 ctx 인터페이스 표면을 줄였다. | `tests/test_pipeline_worker.py`, `tests/test_manifest_view.py`, `tests/test_api_status.py`(90 passed), `pytest -q`(170 passed) | 단계 커밋 revert |
...
4. Wave D (Orchestrator surface reduction): TS-010 (**completed 2026-03-05**), TS-011 (**completed 2026-03-05**)
