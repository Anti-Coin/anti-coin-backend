...
| TS-011 | Orchestrator | `_ctx()` 기반 monkeypatch 호환 래퍼 군 | IN_PROGRESS (2026-03-05) | `D-017` 목표. 조합 책임만 남기기 위해 wrapper 축소 필요. 단, 테스트 계약 동시 이관이 선행 조건이다. 1차로 dead wrapper(`_load/_save_prediction_health`) 제거를 완료했다. | `tests/test_pipeline_worker.py`, `tests/test_manifest_view.py`, `tests/test_api_status.py`(90 passed), `pytest -q`(170 passed) | 단계 커밋 revert |
