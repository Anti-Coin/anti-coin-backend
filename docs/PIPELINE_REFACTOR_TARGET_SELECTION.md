...
| TS-011 | Orchestrator | `_ctx()` 기반 monkeypatch 호환 래퍼 군 | IN_PROGRESS (2026-03-05) | `D-017` 목표. 조합 책임만 남기기 위해 wrapper 축소 필요. 단, 테스트 계약 동시 이관이 선행 조건이다. dead wrapper(`_load/_save_prediction_health`, `_static_export_candidates`, `_extract_updated_at_from_files`, `_query_first/_query_last_timestamp`, `_fetch_ohlcv_paginated`, `_detect_gaps_from_ms_timestamps`, `_refill_detected_gaps`, `save_history_to_json`, `fetch_and_save`) 제거를 완료했고 테스트는 `workers.ingest`/`workers.export` 직접 호출로 이관했다. 추가로 `workers.ingest` 내부에서 `ctx.get_*` 콜백 의존 일부를 모듈 직접 호출(`get_first/last_timestamp`, `get_exchange_latest_closed_timestamp`)로 축소해 ctx 인터페이스 표면을 줄였다. | `tests/test_pipeline_worker.py`, `tests/test_manifest_view.py`, `tests/test_api_status.py`(90 passed), `pytest -q`(170 passed) | 단계 커밋 revert |
