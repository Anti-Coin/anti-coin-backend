...
| D-2026-02-19-40 | Monitor Consistency Scope | monitor consistency는 `symbol+timeframe` only로 고정한다. legacy query fallback은 제거됐다(`D-042`). | query 비용 임계 초과, 오탐/누락 재발 |
...
| D-2026-02-26-67 | D-002 Model Metadata Schema Lock | 모델 metadata/version 스키마를 v1로 잠근다. 저장 경로는 canonical-only `model_{symbol}_{timeframe}.meta.json`이며 필수 필드는 `schema_version/run_id/trained_at/row_count/data_range/model_version/snapshot_path/status`다. primary legacy sidecar 동시 기록 정책은 `D-2026-03-05-81`에서 retire됐다. | metadata 소비 경로(운영 대시보드/승격 게이트)가 확정되거나, schema 확장(예: metrics/drift) 필요가 발생할 때 |
...
| D-2026-03-03-71 | Model Artifact Granularity Clarification | 현재 런타임 모델 아티팩트 단위는 `symbol+timeframe` canonical(`model_{symbol}_{timeframe}.json`)이다. train/predict 모두 canonical-only 경로를 사용하며 primary legacy fallback/sidecar write는 제거됐다(`D-2026-03-05-81`). cross-symbol shared 단일 파일 모델은 현재 코드 경로에 없다. shared/dedicated 분리 승격 정책은 후속(`D-011`)에서 다룬다. | runtime load 경로가 shared 단일 파일 또는 registry resolver로 바뀔 때 |
...
| D-2026-03-04-76 | Scheduler Mode Hard Lock | worker scheduler mode는 env 비노출 `boundary` 단일값으로 잠근다. `poll_loop` 모드와 invalid mode fallback을 제거하고, 잘못된 설정 주입은 fail-fast로 종료한다. | boundary scheduler 장애로 `poll_loop` 재도입 필요가 발생할 때 |
...
| D-2026-03-05-78 | Operator Usecase Baseline Lock | 운영자 실행 계약을 `docs/PIPELINE_OPERATOR_USECASES.md`로 잠근다. stage 흐름은 `preflight -> target -> ingest decision -> ingest execute -> predict eligibility -> predict execute -> export execute -> serve gate`로 고정하고, monitor/alert는 cross-cutting으로 운용한다. 이벤트 최소 스키마는 `stage/cause/impact`를 필수로 한다. canonical 누락은 `fail-closed`, prediction 실패는 `last-good + degraded` 정책을 유지한다. | serving 경계(serve_allowed) 또는 runtime topology(단일 worker/분리 worker) 변경 시 |
| D-2026-03-05-79 | State File Reduction Priority Lock | 상태 파일 축소 우선순위를 잠근다: `prediction_health redundant identity -> symbol_activation redundant/derived -> 기타`. `ingest_watermarks` 제거는 인과성/재시작 경계를 대체하는 설계와 회귀 증거 없이는 진행하지 않는다. | publish gate 인과 경계가 바뀌거나, ingest_state로의 완전 대체 증거가 확보될 때 |
