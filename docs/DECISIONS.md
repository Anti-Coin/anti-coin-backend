...
- Status: Accepted
- Context:
  - 복잡한 분기/조기 반환 로직에서 의도 파악 비용이 높아 세션 전환 시 이해 지연이 발생함
  - 운영 장애 분석 시 상태전이 로그가 부족한 구간이 존재함
- Decision:
  - 핵심 경로에서는 의도 중심 주석(why)을 기본값으로 강화한다.
  - 로그는 상태전이(실패 시작/복구/지속) 중심으로 보강하고, 정상 반복 로그는 과도하게 늘리지 않는다.
  - 위 기준은 A-019로 점진 적용한다.
- Consequence:
  - 코드 리뷰/인수인계 속도가 개선된다.
  - 로그 품질은 올라가지만 로그량이 증가할 수 있어 후속으로 노이즈 점검이 필요하다.
