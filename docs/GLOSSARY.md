...
5. `Hybrid Ingest`: (legacy) base TF direct + derived TF downsample 전략. 현재 활성 정책은 all timeframe direct fetch.
...
13. `external_reconciliation_mismatch`: 외부 대사 불일치. 단발은 `warning`, 동일 bucket 연속 3회면 `critical`.
